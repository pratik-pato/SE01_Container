...
db = MySQLdb.connect("localhost","root","mysql","container" )
...
sql = []
sql.append("SELECT `COLUMN_NAME`  FROM `INFORMATION_SCHEMA`.`COLUMNS`  WHERE `TABLE_SCHEMA`='container'  AND `TABLE_NAME`='%s';"%(sys.argv[1]))
sql.append("select count(*) from %s"%(sys.argv[1]))
